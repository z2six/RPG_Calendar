...
public final class ChronicleDebugCommands {
    ...
    public static void registerGameBus() {
        ...
        try {
            MinecraftForge.EVENT_BUS.addListener(ChronicleDebugCommands::onRegisterCommands);
            REGISTERED = true;
        } catch (Throwable t) {
    ...
    private static int executeFake(CommandSourceStack source, int playerCount, int eventsPerPlayer, int daysRange) {
        ...
        List<Advancement> advancements = collectDisplayAdvancements(server);
        ...
        for (int i = 1; i <= playerCount; i++) {
            ...
            for (int j = 0; j < eventsPerPlayer; j++) {
                Advancement holder = advancements.get(random.nextInt(advancements.size()));
                Advancement adv = holder;
    ...
    private static List<Advancement> collectDisplayAdvancements(MinecraftServer server) {
        List<Advancement> results = new ArrayList<>();
