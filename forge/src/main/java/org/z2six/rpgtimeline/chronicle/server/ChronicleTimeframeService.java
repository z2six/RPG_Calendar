...
public final class ChronicleTimeframeService {
    ...
    private static String resolveAdvancementBackgroundTexture(MinecraftServer server, MatchContext ctx) {
        ...
        while (current != null && visited.add(current)) {
            Advancement holder = server.getAdvancements().getAdvancement(current);
            if (holder == null) {
                break;
            }
            Advancement advancement = holder;
            DisplayInfo display = advancement.getDisplay();
            if (display != null) {
                ResourceLocation bg = resolveBackground(display);
                if (bg != null) {
                    return bg.toString();
                }
            }
            Advancement parent = advancement.getParent();
            current = parent == null ? null : parent.getId();
        }
    ...
    private static String resolveBlockTextureFromItem(ResourceLocation itemId) {
        ...
        if (path.endsWith("_ingot")) {
            ...
            candidates.add(new ResourceLocation(namespace, base + "_block"));
        }
        if (!path.endsWith("_block")) {
            candidates.add(new ResourceLocation(namespace, path + "_block"));
        }
        String material = stripMaterial(path);
        if (!material.isBlank()) {
            candidates.add(new ResourceLocation(namespace, material + "_block"));
    ...
    private static AdvancementInfo resolveAdvancementInfo(MinecraftServer server, ResourceLocation sourceId) {
        ...
        try {
            ...
            Advancement holder = resolveAdvancementHolder(manager, sourceId);
            if (holder == null) {
                return AdvancementInfo.EMPTY;
            }
            DisplayInfo display = holder.getDisplay();
            if (display == null) {
                return AdvancementInfo.EMPTY;
            }
    ...
    private static ResourceLocation resolveBackground(DisplayInfo display) {
        if (display == null) {
            return null;
        }
        Object result = invokeFrame(display, "getBackground");
        if (result == null) {
            result = invokeFrame(display, "background");
        }
    ...
    private static Advancement resolveAdvancementHolder(Object manager, ResourceLocation sourceId) {
        ...
        try {
            ...
            if (result instanceof Advancement holder) {
                return holder;
            }
        } catch (Throwable ignored) {
        }
