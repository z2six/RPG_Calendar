package org.z2six.rpgcalendar.api;

import com.mojang.logging.LogUtils;
import net.minecraft.network.chat.Component;
import org.jetbrains.annotations.NotNull;
import org.slf4j.Logger;
import org.z2six.rpgcalendar.calendar.CalendarDate;
import org.z2six.rpgcalendar.calendar.CalendarDefinition;
import org.z2six.rpgcalendar.calendar.RPGCalendarMath;
import org.z2six.rpgcalendar.platform.Services;

/**
 * Public API entry point for RPG Calendar.
 */
public final class RPGCalendarApi {

    private static final Logger LOG = LogUtils.getLogger();

    private RPGCalendarApi() {
        // no-op
    }
    ...
    public static @NotNull CalendarDefinition getCalendarDefinition() {
        try {
            ...
            LOG.error("[RPGCalendarApi] getCalendarDefinition failed; using default", t);
    ...
    public static @NotNull CalendarDate fromGameTime(long gameTime) {
        return RPGCalendarMath.fromGameTime(gameTime, getCalendarDefinition());
    }
    ...
    public static @NotNull String formatDate(long gameTime) {
        return RPGCalendarMath.formatDate(gameTime, getCalendarDefinition());
    }
    ...
    public static long getDayIndexForGameTime(long gameTime) {
        ...
        if (ticksPerDay <= 0L) {
            LOG.warn("[RPGCalendarApi] ticksPerDay <= 0 ({}), using 24000 fallback", ticksPerDay);
            ticksPerDay = 24000L;
        }
    ...
    public static @NotNull Component buildDateMessage(long dayIndex) {
        try {
            ...
            if (daysPerMonth <= 0) {
                LOG.warn("[RPGCalendarApi] buildDateMessage: daysPerMonth <= 0 ({}), falling back to 28", daysPerMonth);
                daysPerMonth = 28;
            }
            if (monthsPerYear <= 0) {
                LOG.warn("[RPGCalendarApi] buildDateMessage: monthsPerYear <= 0 ({}), falling back to 8", monthsPerYear);
                monthsPerYear = 8;
            }

            long totalDaysPerYear = (long) daysPerMonth * (long) monthsPerYear;
            if (totalDaysPerYear <= 0L) {
                LOG.warn("[RPGCalendarApi] buildDateMessage: totalDaysPerYear <= 0, forcing safe fallback");
                totalDaysPerYear = (long) daysPerMonth * 8L;
            }

            if (dayIndex < 0L) {
                LOG.warn("[RPGCalendarApi] buildDateMessage: dayIndex < 0 ({}), clamping to 0", dayIndex);
                dayIndex = 0L;
            }
            ...
            LOG.error("[RPGCalendarApi] buildDateMessage failed, using fallback text", t);
