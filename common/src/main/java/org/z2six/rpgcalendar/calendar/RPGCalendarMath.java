 ...
 */
public final class RPGCalendarMath {

    private static final Logger LOG = LogUtils.getLogger();

    private RPGCalendarMath() {
        // no instances
    }
    ...
    public static CalendarDate fromGameTime(long gameTime, @NotNull CalendarDefinition definition) {
        try {
            ...
            if (ticksPerDay <= 0L) {
                LOG.warn("[RPGCalendarMath] ticksPerDay <= 0 ({}), falling back to 24000", ticksPerDay);
                ticksPerDay = 24000L;
            }
            ...
            if (daysPerMonth <= 0) {
                LOG.warn("[RPGCalendarMath] daysPerMonth <= 0 ({}), falling back to 28", daysPerMonth);
                daysPerMonth = 28;
            }
            ...
            LOG.error("[RPGCalendarMath] fromGameTime failed, returning default date for 0 AN", t);
            return new CalendarDate(
                    0L,
                    0L,
                    0L,
